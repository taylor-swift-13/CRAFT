...
\(I(C)=\bigwedge_{c\in C}c\), with \(I(\varnothing)=\mathsf{true}\).  We
distinguish three standard partial-correctness obligations
\citep{floyd1967assigning,hoare1969axiomatic,cao2025clause2inv}:
\begin{align}
\operatorname{Est}_{\mathcal L}(I(C))
&\iff \forall\sigma:\ \mathsf{Init}(\sigma)\Rightarrow I(C)(\sigma),
&&\text{(establishment)},\label{eq:method-establishment}\\
\operatorname{Pres}_{\mathcal L}(I(C))
&\iff \forall\sigma,\sigma':\
I(C)(\sigma)\land B(\sigma)\land S(\sigma,\sigma')
\Rightarrow I(C)(\sigma'),
&&\text{(preservation)}.\label{eq:method-preservation}
\end{align}
Preservation is the inductiveness condition.  A valid loop invariant must
satisfy both establishment and preservation:
\[
\operatorname{ValidInv}_{\mathcal L}(I(C))
\iff \operatorname{Est}_{\mathcal L}(I(C))
\land \operatorname{Pres}_{\mathcal L}(I(C)).
\]

For a target \((\ell,g)\), let \(R_\ell\) relate a loop-head state to a state
at \(\ell\) along an execution path.  The termination (postcondition) obligation
is
\begin{equation}
\operatorname{Term}_{\mathcal L}(I(C),\ell,g)\iff
\forall\sigma,\sigma':\
I(C)(\sigma)\land R_\ell(\sigma,\sigma')\Rightarrow g(\sigma').
\label{eq:method-termination}
\end{equation}
The task is verified when \(\operatorname{ValidInv}_{\mathcal L}(I(C))\)
holds and this obligation holds for every target in \(\mathcal G\).  The
termination obligation states what follows if execution reaches the target;
it does not prove that the loop terminates.  Appendix~\ref{app:formal-loop-verification}
defines \(R_\ell\) for targets inside and after the loop, including the
applicable exit paths.

In the \emph{target-hidden} setting, generation and all intermediate scores
observe only \(\mathcal L\); \(\mathcal G\) is restored only for final
verification.  Preconditions and executable loop semantics remain visible.
We study scalar-integer C programs with one loop; Appendix~\ref{app:scope-example}
gives the scope and an example.
\end{definition}
...
from a fixed generator policy \(\pi_0\) (instantiated in
Section~\ref{sec:exp:setup}), adds their clauses
