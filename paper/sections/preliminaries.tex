...
\(S^B_\ell\) for the path-sensitive prefix from the loop head through
guard evaluation to body location \(\ell\),
...
\begin{align}
 I(\sigma)\land S^B_\ell(\sigma,\sigma')
...
\(X_\ell\) includes both guard-false exits and body prefixes leading to
\texttt{break}; Appendix~\ref{app:formal-loop-verification} defines it explicitly.
...
Generation and intermediate scores depend only on the visible loop context
\(\mathcal L\).
