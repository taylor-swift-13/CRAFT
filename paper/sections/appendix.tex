...
Execution-trace sampling belongs only to reward construction, while grouped model rollouts, monotone refinement pools, and the Houdini backend are shared with inference.
Inference has its own refinement budget and never constructs a trace example set.
...
\midrule
$(w_b, w_h)$ & $(1.0, 0.3)$ & standalone coverage / group-hardness weights \\
$w_r$ & 0.02 & cost per conservative semantic duplicate; unique zero-coverage clauses are not penalized \\
...
returns both the nonnegative diagnostic \(\Delta\mathrm{base}\) and
