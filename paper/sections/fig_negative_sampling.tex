...
\begin{tikzpicture}[
  ...
  panel/.style={rounded corners=6pt, fill=algobg!58, draw=promptframe,
    line width=0.8pt},
  subcard/.style={rounded corners=3.5pt, fill=white, line width=0.8pt},
  stage/.style={font=\scriptsize\bfseries, text=codegray},
  ttl/.style={font=\scriptsize\bfseries, text=paperink, anchor=west},
  ann/.style={font=\tiny, text=codegray, align=center},
...
\node[ann, text=paperink] at (1.30,0.88)
  {masked loop\\[-1pt]{\color{codered}\(Q\) hidden}};
...
% relation family
\draw[subcard, draw=codepurple!50] (3.55,0.12) rectangle (9.75,1.66);
\node[ttl] at (3.75,1.42) {Relation};
...
% post-exit family
\draw[subcard, draw=codeblue!50] (3.55,-1.66) rectangle (9.75,-0.12);
\node[ttl] at (3.75,-0.38) {Post-exit};
...
\node[ann, text=paperink] at (12.10,-0.62)
  {\textbf{60 total} matched traces};
\node[ann] at (12.10,-0.98) {relation + post-exit + random fill};

\draw[flow] (13.50,0) -- (14.20,0);

%% ---------------- Output ----------------
\node[circle, draw=codered!70, fill=wrongred!22, line width=1.1pt,
...
cleaning selects these traces first, then random perturbations fill the common
60-trace budget.  The resulting \(\mathcal N\) is used for SFT acceptance and
RL reward computation and never exposes \(Q\).}
