...
Aligning SFT targets and RL rewards with this inference process further improves
...
\caption{Verification versus tokens on each method's full supported set.
Direct GPT-5-nano results use 832 tasks, except Clause2Inv (316 Linear) and
LaM4Inv (366 Linear+NLA). The dashed trained-CRAFT reference uses all 832.
Squares mark the new LORIS and LaM4Inv adaptations.}
...
For LaM4Inv, Clause2Inv, and LORIS, these are target-hidden adaptations,
not reproductions of their native target-visible configurations.
Each method is evaluated on its full supported set. LORIS and the other
applicable baselines use all 832 programs. LaM4Inv uses 316 Linear and 50 NLA
programs because its required Loopy CFG/SMT artifacts are unavailable;
Clause2Inv uses the 316 Linear programs supported by its released VC
interface. Omitted categories are not counted as failures.
Appendix~\ref{app:tools-comparison} reports matched-subset CRAFT comparisons
and explains why Quokka/InvBench is not directly comparable.
...
exceeding AutoSpec (42.19\%, 2,216 tokens) and LORIS (42.55\%, 47,816 tokens).
Eight responses raise verification to 55.53\% at 2,929 tokens. On their
smaller supported subsets, LaM4Inv reaches 19.40\% and Clause2Inv reaches
20.89\%; matched CRAFT rates appear in Appendix~\ref{app:tools-comparison}.
...
trade-off. Under the uniform target-hidden adaptation, compose@4 outperforms
LORIS, LaM4Inv, and Clause2Inv on their respective matched supported sets,
while using substantially fewer tokens than LORIS.
\end{insightbox}
