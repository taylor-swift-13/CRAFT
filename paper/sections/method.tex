...
\begin{definition}[Target-hidden loop-verification task]
\label{def:method-task}
Let \(\mathcal L=(\mathsf{Init},B,S)\) describe loop-entry states, the guard,
and one loop step.  A task \(\mathcal T=(\mathcal L,\mathcal G)\) additionally
specifies verification targets \(\mathcal G\) at their program locations.
For a clause set \(C\), the composed invariant is
\(I(C)=\bigwedge_{c\in C}c\), with \(I(\varnothing)=\mathsf{true}\).
Its establishment and preservation conditions are
\begin{align*}
...
\end{align*}
The invariant is valid exactly when both hold:
\(\operatorname{ValidInv}_{\mathcal L}(I(C))\iff
\operatorname{Est}_{\mathcal L}(I(C))\land
\operatorname{Pres}_{\mathcal L}(I(C))\).
In the \emph{target-hidden} setting, generation and intermediate scores see
\(\mathcal L\) but not \(\mathcal G\); targets are used only for final
verification.  Appendix~\ref{app:formal-loop-verification} gives the full
proof obligations, and Appendix~\ref{app:scope-example} illustrates the
numerical-loop setting through case studies.
\end{definition}
