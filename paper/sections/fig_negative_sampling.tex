...
\begin{tikzpicture}[
  ...
  >=Stealth,
  flow/.style={-{Stealth[length=1.6mm,width=1.0mm]}, draw=paperink!58, line width=0.75pt},
  ...
  pdot/.style={paper state, fill=codegreen},
  neg/.style={paper state, draw=codered, fill=wrongred!35, line width=0.6pt},
  pedge/.style={-{Stealth[length=1.6mm,width=1.0mm]}, draw=codegreen!75, line width=0.75pt},
  nedge/.style={-{Stealth[length=1.6mm,width=1.0mm]}, draw=codered!80, line width=0.75pt},
  perturb/.style={-{Stealth[length=1.6mm,width=1.0mm]}, draw=codered!80, line width=0.75pt,
    dashed}
]
...
\node[ann, text=paperink] at (1.30,0.88)
  {masked loop\\[-1pt]{\color{codegray}\(\mathcal G\) hidden}};
...
% off-trajectory family
\draw[subcard] (3.55,0.12) rectangle (9.75,1.66);
\node[ttl, text=paperink] at (3.75,1.42) {Off-trajectory};
...
\draw[pedge] (p1) -- (p2);
\draw[pedge] (p2) -- (p3);
\draw[dashed, draw=codegray!85, line width=0.75pt] (6.15,-1.50) -- (6.15,-0.62);
...
\node[ann, text=paperink] at (12.10,-0.62)
  {48 off-trajectory \(\cdot\) 12 post-exit};
...
probe variable constraints and behavior past a genuine exit.  Conservative
cleaning and per-family budgets select candidates from both transformations;
local perturbations fill unused capacity (not shown).
