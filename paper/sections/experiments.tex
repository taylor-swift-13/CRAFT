...
pass@\(k\) counts success when at least one response independently verifies
the targets; compose@\(k\) counts success after filtering and composing
their clauses. Appendix~\ref{app:experimental-protocol} gives the definitions.
...
at one- and eight-response budgets. RL denotes training from Base;
...
For Base Qwen3-8B, pass@1 is 6.37\%,
compose@1 reaches 37.86\%, or \(5.94\times\) the standalone rate.
...
% ---------------------------------------------------------------------------
\subsection{RQ4: Why Do Composed SFT Labels and Clause-Level Rewards Help?}
...
\captionsetup{font=small,margin={7pt,0pt}}
\caption{Qwen3-8B reward ablation from Base: (a) pass@\(k\); (b) compose@\(k\). Vertical scales differ.}
...
of training examples, composed-label SFT raises Qwen3-8B pass@1 from
19.95\% to 41.95\% and compose@1 from 23.20\% to 63.94\%, relative to
