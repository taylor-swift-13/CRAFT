...
\begingroup
\definecolor{ovInk}{HTML}{243247}
\definecolor{ovMuted}{HTML}{64748B}
\definecolor{ovBorder}{HTML}{DCE7E2}
\definecolor{ovGreen}{HTML}{147D68}
\definecolor{ovReject}{HTML}{69788A}
\definecolor{ovPaper}{HTML}{F7FAF8}
\pgfmathsetlengthmacro{\ovunit}{\textwidth/16}
\pgfmathsetlengthmacro{\ovrewardwidth}{6.41*\ovunit-6pt}
\pgfmathsetlengthmacro{\ovrlwidth}{3.90*\ovunit-6pt}
% Keep all labels at their native font size; the vertical canvas adds breathing room.
\def\ovbody{\sffamily\fontsize{9.0}{10.6}\selectfont}
\def\ovtitle{\sffamily\fontsize{9.5}{11.1}\selectfont\bfseries}
\begin{tikzpicture}[
  x=\ovunit, y=1.02*\ovunit, font=\ovbody, text=ovInk,
  >=Stealth,
  flow/.style={-{Stealth[length=1.35mm,width=.9mm]},
    draw=ovMuted, line width=.55pt},
  feedback/.style={-{Stealth[length=1.35mm,width=.9mm]},
    draw=ovGreen!85!black, line width=.55pt,
    dash pattern=on 2pt off 1.4pt, rounded corners=1.5pt},
  credit/.style={-{Stealth[length=.95mm,width=.65mm]},
    draw=ovMuted!72, line width=.35pt, dash pattern=on 1.4pt off 1pt},
  panel/.style={draw=ovBorder, fill=white, rounded corners=2pt,
    line width=.45pt},
  card/.style={draw=ovBorder, fill=white, rounded corners=1.5pt,
    line width=.4pt, align=center, inner xsep=3pt, inner ysep=4pt,
    outer sep=0pt},
  ...
  u/.style={chip, fill=ovReject!16, draw=ovReject!55, text=ovReject!85!black},
  v/.style={u},
  pool/.style={rounded corners=1.5pt, draw=ovMuted!65, fill=white,
    line width=.45pt, dash pattern=on 2.3pt off 1.5pt},
  state/.style={circle, draw=ovGreen, fill=ovGreen,
    minimum size=.18*\ovunit, inner sep=0pt},
  candidate/.style={circle, draw=ovReject, fill=ovReject!16,
    line width=.55pt, minimum size=.20*\ovunit, inner sep=0pt}
]
...
\node[card, draw=none, fill=black!5, text width=2.00*\ovunit,
  inner ysep=3pt, minimum height=2.2184*\ovunit] (masked) at (1.40,5.14)
  {Masked loop\\[2pt]\(\mathcal L\! =\! (\mathrm{Init},B,S)\)\\[2pt]
   \textcolor{ovMuted}{\(\mathcal G\) hidden}};
\fill[ovGreen!4, rounded corners=2pt] (2.86,3.96) rectangle (8.21,6.32);
\draw[flow] (masked.east |- 0,4.94)--(2.82,4.94);
...
\foreach \y/\lab/\clauses in {
  ...
  \foreach \letter [count=\i from 0] in \clauses {
    ...
    \node[\letter] at (6.65+\i*.34,\y) {\(\letter\)};
  }
  \node[note, anchor=east] at (6.34,\y) {\(A_{\lab}\)};
}
\draw[ovGreen, line width=.8pt]
  (5.29,5.38)--(5.34,5.32)--(5.44,5.47);
\foreach \y in {4.94,4.50}{
  \draw[ovReject, line width=.8pt]
    (5.29,\y-.07)--(5.43,\y+.07)
    (5.29,\y+.07)--(5.43,\y-.07);
}
\draw[flow] (8.25,4.94)--(8.58,4.94);

\fill[ovGreen!4, rounded corners=2pt] (8.66,3.96) rectangle (15.60,6.32);
...
% (b) SFT: original acceptance, resampling, labels, and fine-tuning.
\draw[panel] (.10,.10) rectangle (3.96,7.05);
\node[heading, anchor=west] at (.38,6.76) {(b)\; SFT};
\node[note] at (2.03,6.17) {compose \(\to\) label};
\draw[ovBorder, line width=.4pt] (.38,6.48)--(3.68,6.48);
\node[card, draw=none, fill=black!5, text width=2.82*\ovunit] (composed) at (2.12,5.14)
  {Composed set\\[1pt]\(C_{\mathcal L}^{(t)}\)};
\node[card, draw=none, fill=black!5, text width=2.62*\ovunit] (accept) at (2.12,3.49)
  {Coverage \(\ge\tau\)?};
...
\node[card, fill=ovGreen, draw=none, text=white,
  font=\ovtitle, text width=2.82*\ovunit] (sfttarget) at (2.12,2.02) {SFT labels};
\draw[flow] (accept.south)--(sfttarget.north);
\node[note, anchor=west] at (2.24,2.77) {yes};
\node[card, draw=none, fill=black!5, text width=2.82*\ovunit] (sftmodel) at (2.12,.74) {Fine-tune};
...
\node[heading, anchor=west] at (4.58,6.12) {Binary};
\node[note, anchor=west] at (5.95,6.12)
  {\(\vcenter{\hbox{\sffamily valid set}}\mapsto\{0,1\}\)};

% A small, separate inset shows the two candidate-negative mechanisms.
\fill[ovGreen!4, rounded corners=2pt] (4.52,3.90) rectangle (10.93,5.90);
\node[heading, anchor=west] at (4.58,5.63) {Candidate negatives};
...
\draw[flow, draw=ovReject, dashed] (off2)--(offneg);
...
\draw[flow, draw=ovReject] (post2)--(post3);
\draw[flow, draw=ovReject] (post3)--(post4);
\node[circle, draw=ovReject!65, fill=white,
  minimum size=.49*\ovunit, inner sep=0pt] (negset) at (10.29,4.79)
  {\(\mathcal N\)};
\draw[flow, draw=ovReject!80, rounded corners=2pt]
  (offneg.east)--(9.35,5.31)--(negset.north west);
\draw[flow, draw=ovReject!80] (post4.east)--(negset.south west);
\draw[flow, rounded corners=2pt]
...
\node[card, draw=ovGreen!40, fill=ovGreen!5,
  text width=\ovrewardwidth,
  inner ysep=2.5pt] (rewardformula) at (7.725,.74)
...
\foreach \y/\lab/\clauses in {
  ...
  \foreach \letter [count=\i from 0] in \clauses {
    \node[\letter, draw=none] at (12.77+\i*.41,\y) {\(\letter\)};
  }
}
\draw[flow] (13.60,4.59)--(13.60,4.40);
\foreach \letter [count=\i from 0] in {a,b,c,d,e,f}{
  \node[\letter, draw=none] (owner\letter) at (12.54+\i*.44,3.84) {\(\letter\)};
}
\node[note] at (13.70,4.20) {pool, filtered once};
...
\node[card, draw=none, fill=black!5, text width=\ovrlwidth, inner ysep=3pt] (grpo) at (13.70,1.18)
